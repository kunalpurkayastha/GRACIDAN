...
class GAttN(nn.Module):
    def __init__(self, in_channels, out_channels, heads=8, concat=True, dropout=0.6):
        ...
        Args:
            ...
            heads: Number of attention heads in the GAT.
            concat: Whether to concatenate or average the heads' outputs.
        ...
        # Linear transformation layers for computing query, key, and value
        self.query_transform = nn.Linear(in_channels, out_channels * heads, bias=False)
        self.key_transform = nn.Linear(in_channels, out_channels * heads, bias=False)
        self.value_transform = nn.Linear(in_channels, out_channels * heads, bias=False)

        # Attention mechanism scaling factor
        self.scale = 1.0 / (out_channels ** 0.5)

        # Optional output transformation for combining attention heads
        if concat:
            self.output_transform = nn.Linear(out_channels * heads, out_channels)
        else:
    ...
    def forward(self, x, edge_index):
        """
        Forward pass for GAttN.

        Args:
            x: Input feature tensor of shape (B, N, in_channels), where N is the number of nodes.
            edge_index: Graph connectivity information (2, num_edges), where each column defines an edge.

        Returns:
            Output tensor of shape (B, N, out_channels).
        """
        B, N, C = x.size()  # Batch size, number of nodes, input channels

        # Apply linear transformations to get query, key, value
        query = self.query_transform(x).view(B, N, self.heads, self.out_channels)
        key = self.key_transform(x).view(B, N, self.heads, self.out_channels)
        value = self.value_transform(x).view(B, N, self.heads, self.out_channels)
        ...
        out = torch.einsum('bhqk,bhkd->bhqd', attention_weights, value)
...
def construct_edge_index(H, W):
    """
    Construct a simple grid-based edge index for a graph.
    ...
    Returns:
        edge_index: A tensor representing the graph edges.
    """
    edge_index = []
    for i in range(H):
        for j in range(W):
            idx = i * W + j
            # Add edges for 4-connected neighbors (left, right, top, bottom)
            if i > 0:  # Top
...
class MaskRCNNConvUpsampleHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: ShapeSpec):
        ...
        self.gat = GAttN(in_channels=input_shape.channels, out_channels=cfg.MODEL.ROI_MASK_HEAD.CONV_DIM)

        self.boundary_gat_bo = GAttN(in_channels=input_shape.channels, out_channels=cfg.MODEL.ROI_MASK_HEAD.CONV_DIM)
        self.boundary_gat = GAttN(in_channels=input_shape.channels, out_channels=cfg.MODEL.ROI_MASK_HEAD.CONV_DIM)
    ...
    def forward(self, x):
      B, C, H, W = x.size()
      x_ori = x.clone()
      edge_index = construct_edge_index(H, W).to(x.device)


      # First branch for boundary occlusion prediction (replacing with GAttN)
      for cnt, layer in enumerate(self.boundary_conv_norm_relus):
          x = layer(x)

          if cnt == 1 and len(x) != 0:
              # Reshape input for GAttN
              x_query_bound_bo = x.view(B, -1, C)  # Shape (B, HW, C), where HW = H * W

              # Pass through the GAttN layer instead of manual attention mechanism
              x_query_bound_bo = self.gat(x_query_bound_bo, edge_index)
              x_query_bound_bo = x_query_bound_bo.view(B, C, H, W)  # Reshape back to (B, C, H, W)

              # Combine with the original input
              x = x + x_query_bound_bo
      ...
      # Second branch for boundary prediction
      for cnt, layer in enumerate(self.conv_norm_relus):
          x = layer(x)
          if cnt == 1 and len(x) != 0:
              # Reshape input for GAttN
              x_query_bound = x.view(B, -1, C)  # Shape (B, HW, C), where HW = H * W

              # Pass through the GAttN layer instead of manual attention mechanism
              x_query_bound = self.gat(x_query_bound, edge_index)
              x_query_bound = x_query_bound.view(B, C, H, W)  # Reshape back to (B, C, H, W)
      ...
      # Deconvolution and mask prediction remain the same
