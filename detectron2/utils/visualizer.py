...
class VisImage:
    ...
    def get_image(self):
       ...
       # except ImportError:
        ...
        #print('shape1:', img * (1 - alpha).shape)
        #print('shape2:', rgb * alpha.shape)
        if len(img.shape) == 3:
...
class Visualizer:
    ...
    *,
    ...
    alpha=1.0  # Set alpha to 1.0 for full opacity
):
      num_instances = None
      ...
      # Set all colors to white and background to black
      assigned_colors = [[1.0, 1.0, 1.0] for _ in range(num_instances)]
      self.output.img = np.zeros_like(self.output.img)  # Set background to black
      ...
      for i in range(num_instances):
          ...
          if masks is not None:
              for segment in masks[i].polygons:
                  self.draw_polygon(segment.reshape(-1, 2), color, alpha=1.0)  # Full opacity
    ...
    self, binary_mask, color=None, *, edge_color=None, text=None, alpha=0.5, area_threshold=4096
      ...
      Args:
          ...
          color: color of the mask. Refer to `matplotlib.colors` for a full list of
              formats that are accepted. If None, the color will be white.
      ...
      self.output.img = np.zeros_like(self.output.img, dtype=np.uint8)
      ...
      if not mask.has_holes:
          ...
          for segment in mask.polygons:
              area = mask_util.area(mask_util.frPyObjects([segment], mask.height, mask.width))
          ...
          rgba[:, :, :3] = color  # Set color for the mask
          rgba[:, :, 3] = (mask.mask == 1).astype("float32") * alpha  # Apply alpha transparency
          has_valid_segment = True
          self.output.ax.imshow(rgba)

      # Optionally, draw text inside the mask (you can remove this if not needed)
