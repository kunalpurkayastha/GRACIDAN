...
class MBConv(nn.Module):
    """Mobile Inverted Bottleneck Convolution Block with SE (Squeeze-and-Excitation)."""

    def __init__(self, c1, c2, kernel_size=3, stride=1, expand_ratio=4, se_ratio=0.25, activation='swish'):
        ...
        # Expansion phase
        if expand_ratio != 1:
            self.expand_conv = nn.Conv2d(c1, hidden_dim, kernel_size=1, stride=1, bias=False)
        ...
        # Output phase
        self.project_conv = nn.Conv2d(hidden_dim, c2, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        # Depthwise convolution
        x = self.act2(self.bn2(self.depthwise_conv(x)))
...
#     def forward_fuse(self, x):
#         """Apply fused convolution directly."""
#         return self.act(self.conv(x))
